...
const getAccountsGraphs = `-- name: GetAccountsGraphs :one
SELECT COUNT(*) FROM accounts WHERE user_id = $1 AND type = $2
`

type GetAccountsGraphsParams struct {
	UserID int32  `json:"user_id"`
	Type   string `json:"type"`
}

func (q *Queries) GetAccountsGraphs(ctx context.Context, arg GetAccountsGraphsParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, getAccountsGraphs, arg.UserID, arg.Type)
	var count int64
	err := row.Scan(&count)
	return count, err
}
